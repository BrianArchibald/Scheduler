...
function initWeekCalendar(x) {
  ...
  for (let i = n * 7; i < n * 7 + 7; i++) {
    ...
    weekHtml += `<div class="booking-day">
				<div class="booking-day-title">${dayOfWeek}</div>
					<button class="booking-day-button" data-date="${date
            .addDays(i)
            .getDate()}" data-month="${dayOfMonth}" data-year="${currentYear}" onclick="activeClass(this, $(this))">
...
$(".booking-time-button").click(function() {
  ...
  if ($("#booking-calendar .active-button").length)
    activeClass(
      document.querySelector("#booking-calendar .active-button"),
      clicked
    );
});
...
function activeClass(el, target) {
  ...
  // Add active class to button clicked
  if (
    clicked.hasClass("active-button") &&
    !target.hasClass("booking-time-button")
  ) {
  ...
  if (start != 0 && end != 0) {
    ...
    if (seconds) {
      do {
        ...
        let formattedTime = startFormatted.toLocaleString("en-US", {
          hour: "numeric",
          minute: "numeric",
          hour12: true
        });
        startHtml += `<button class="booking-time-selector booking-button" data-hour-min=${formattedTime} onclick="activeClassSpecificTime(this)">${formattedTime}</button>`;
